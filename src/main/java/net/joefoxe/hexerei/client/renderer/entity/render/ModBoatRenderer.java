...
public class ModBoatRenderer extends EntityRenderer<ModBoatEntity> {
    ...
    public void render(ModBoatEntity pEntity, float pEntityYaw, float pPartialTicks, PoseStack pMatrixStack, MultiBufferSource pBuffer, int pPackedLight) {
        ...
        pMatrixStack.mulPose(Vector3f.YP.rotationDegreesf(180.0F - pEntityYaw));
        ...
        if (f > 0.0F) {
            pMatrixStack.mulPose(Vector3f.XP.rotationDegreesf(Mth.sin(f) * f * f1 / 10.0F * (float)pEntity.getHurtDir()));
        }
        ...
        pMatrixStack.mulPose(Vector3f.YP.rotationDegreesf(90.0F));
